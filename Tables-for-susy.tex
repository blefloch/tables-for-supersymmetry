...
\paragraph{q-theta function} $\theta(z;q)=(z;q)_\infty(q/z;q)_\infty$ obeys $\theta(z;q)=\theta(q/z;q)=-z\theta(1/z;q)$.

\paragraph{Dedekind eta function.}  Modular form of weight~$\frac{1}{2}$ given by $\eta(\tau)=q^{1/24}(q;q)_\infty$ where $q=\E^{2\pi\I\tau}$.

\paragraph{Theta function}  $\vartheta_1(x;q)=-\I x^{1/2}q^{1/8}(q;q)_\infty(qx;q)_\infty(\frac{1}{x};q)_\infty$.
