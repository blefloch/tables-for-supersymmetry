...
% todo: section on representations of complex Lie algebras
% todo: representations of real Lie algebras

% todo: section on Lie groups here
% todo: Name "USp(2n)=Sp(n)" the compact symplectic group
...
\begin{tab}{@{ }l@{ \ }l@{ \ }l@{ }}
  Manifold type & $G$-structure & Other characterization\tabfoot{1} \\
  \midrule
  Riemannian & $\Lie{O}(n)$ & Symmetric metric $g>0$ \\
  Almost complex & $\Lie{GL}(n/2,\CC)$ & $\CC\actson TX$ (\ie, $J^2=-1$) \\
  Almost symplectic & $\Lie{Sp}(2n/2,\RR)$ & Non-degenerate $\omega\in\Omega^2X$\\
  Almost Hermitian & $\Lie{U}(n/2)$ & Two compatible $(\omega,J,g)$\tabfoot{3} \\
  \rlap{Almost hypercomplex\tabfoot{2} $\Lie{U}^*(n/2)$} & & $\HH\actson TX$ \\
\end{tab}
\noindent\tabfoot{1} All sections are global.  For instance an almost complex
structure is a global section $J$~of $\End TX$ with $J^2=-1$.

% todo: in the table, which of almost (quaternionic|hyper)(complex|Hermitian) to include?

\noindent\tabfoot{2} Almost hypercomplex manifolds, also called
quaternionic, require global $J_1,J_2,J_3:TX\to TX$ with
$J_i^2=J_1J_2J_3=-1$.
...
\paragraph{Integrability.} A $G$-structure is $k$-integrable (resp.\@ integrable) near
...
\end{tikzpicture}
\end{tab}
(Almost) hyper-(complex/Hermitian/K\"ahler) manifolds are defined by
replacing~$J$ by a triplet with
$J_i^2=J_1J_2J_3=-1$.  (Almost)
quaternionic-(complex/Hermitian/K\"ahler) allow the $J_i$ to be locally
defined but spanning a globally defined 3d~subbundle of $\End TX$.
% ref: math/0105206

% \noindent
% (Almost) quaternionic-(complex/Hermitian/K\"ahler) manifolds are defined
% by replacing~$J$ by a 3d~subbundle of $\End TX$
% with a local basis of sections~$J_i$ obeying $J_i^2=J_1J_2J_3=-1$.
% (Almost) hyper-(complex/Hermitian/K\"ahler) manifolds require global
% sections~$J_i$.


% \noindent
% Analogous manifolds with $J$~replaced by three almost complex structures
% with $J_1J_2=J_3$, are called (almost)
% hyper-(complex/Hermitian/K\"ahler).  Allowing the $J_i$ to only be local
% but spanning a globally defined three-dimensional subbundle of $\End TX$
% yields (almost) quaternionic-(complex/Hermitian/K\"ahler) manifolds.

% the (almost) complex

%  with the action
% of~$\mathbb{C}$ on tangent spaces replaced by an action the of
% quaternions~$\mathbb{H}$, namely three (almost) complex structures $I$,
% $J$, $K$ with $IJ=K$.  The manifolds are called (almost) hypercomplex,
% (almost) .

\noindent\tabfoot{1}
Since $\Lie{GL}(n,\RR)/O(n)$ is contractible, any manifold admits
(non-canonically) an $O(n)$-structure, namely a smooth choice of
...
% todo: HyperK\"ahler manifolds admit a holomorphic symplectic form
...
\section{Data for supersymmetric gauge theories}
...
\section{String theory dualities}
