...
For $8$~supercharges, symplectic representation $V\simeq\HH^n$ namely $G\to\Lie{USp}(2n)$.
For $4$~supercharges, unitary representation $V\simeq\CC^n$ namely $G\to\Lie{U}(n)$.
...
When it is abelian, the IR R-symmetry (that is part of the superconformal algebra) can be a mixture of the UV R-symmetry with abelian flavour symmetries.
This is fixed in 4d $\Nsusy=1$ by $a$-extremization, in 3d $\Nsusy=2$ by $Z_{S^3}$-extremization, in 2d $\Nsusy=(0,2)$ by $c$-extremization, assuming no accidental flavour symmetries.
...
The Higgs branch has flavour symmetry $C(G)/Z(G)$ commutant of $G$ in $U(R)$ or $USp(\widetilde{R})$ modulo center of~$G$.
...
$\Lie{SU}(2N)$ SYM with $\Nf\leq 7$ fundamental hypermultiplets has $\Lie{SO}(2\Nf)\times\Lie{U}(1)_T\subset\Lie{E}_{\Nf+1}$ flavor symmetry enhancement.
...
In any case the R-symmetry is abelian hence may mix with flavour symmetries to give IR R-symmetry.
