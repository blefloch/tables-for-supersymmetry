...
\paragraph[Identity component]{The identity component~$G_0$} is the connected component of~$G$ containing the identity.

Any compact connected Lie group~$G$ is a quotient of $\torus^q\times\prod_i G_i$ by a finite subgroup of its center, with Lie groups~$G_i$ that are simple, compact, simply-connected and connected.

\paragraph{Homotopy.} Every (connected?) Lie group~$G$ deformation-retracts onto its maximal compact subgroup (hence is homotopic to it), has $\pi_1(G)$ abelian and finitely-generated, $\pi_2(G)=0$, and $\pi_3(G)\sim\ZZ^{\text{finite}}$ (Iwasawa).  Simple Lie groups~$G$ have $\pi_3(G)=\ZZ$.

% todo: more on homotopy groups of Lie (super)groups

Then $\pi_n(G)=\prod_i \pi_n(G_i)$ for $n\geq 2$.
