...
\usepackage[paperwidth=282.230648pt,paperheight=399.13441pt,width=268.00427pt,top=7.113189pt,bottom=15pt,footskip=10pt]{geometry}
\usepackage{xcolor}
...
\DeclareMathOperator{\pllog}{pllog}
...
\ExplSyntaxOff
\newcommand{\hepthaux}[1]{\href{https://arxiv.org/abs/hep-th/#1}{hep-th/#1}}
...
\noindent{\scriptsize Bruno Le Floch, \'Ecole Normale Sup\'erieure, 2019.\\
...
\paragraph{Global symmetries} include Poincar\'e or conformal, flavour symmetries, R-symmetries.
Not always all manifest.  In a given QFT description, ``electric''  flavour symmetries act on matter.

\paragraph{Topological/magnetic flavour symmetry} of a gauge field with gauge group~$\Lie{G}$:
$\Hom(\pi_1(\Lie{G}),\Lie{U}(1))$.

\paragraph{An emergent symmetry} is a flavour symmetry of an IR CFT, absent in some UV description.
...
The (electric) flavour symmetry is then $\{x\in F\mid xG=Gx,xH=Hx\}/H$.

\paragraph{An abelian \(q\)-form symmetry} \arxiv{1412.5148}
(for $q=0$ this is a standard flavour symmetry and can be nonabelian)
...
For nonabelian R-symmetry that flavour symmetry must be emergent as it does not commute with~$R_{\text{UV}}$.
For abelian R-symmetry the mixing is continuous; assuming no emergent flavour symmetries it is fixed
...
\paragraph{Resurgence} see \arxiv{1909.05262} in supersymmetric theories.

\section{Spinors, supersymmetry}
...
\subsection{Unitary SCFT irreps}

See \arxiv{1612.00809}, in Lorentzian signature;
$[j_1,\dots]_\Delta^{(R_1,\dots)}$ denotes a conformal family whose primary has Dynkin labels $\vec{j}$ and $\vec{R}$ (in $\ZZ_{\geq 0}$) for Lorentz and R-symmetry, and dimension~$\Delta$;
superconformal multiplets are long $L[\vec{j}]_\Delta^{(\vec{R})}$ with $\Delta\geq\Delta_A(\vec{j},\vec{R})$ or short $X_\ell[\vec{j}]_\Delta^{(\vec{R})}$ with $X\in\{A,B,C,D\}$ and $\Delta=\Delta_X(\vec{j},\vec{R})$ and $\ell=\ell_X(\vec{j},\vec{R})\in\ZZ_{\geq 0}$ and $\Delta_A>\Delta_B>\Delta_C>\Delta_D$.

\paragraph{3d \(\Nsusy=1\) algebra \(\lie{osp}(1|4)\).}
Supercharges $Q\in [1]_{1/2}$.
Let $\Delta_A(0)=\frac{1}{2}$ and $\Delta_A(j\geq 1)=\frac{1}{2}j+1$.
Multiplets: $L[j]_{\Delta>\Delta_A}$, $A_{1+\delta_{j0}}[j]_{\Delta_A}$, $B_1[0]_0$.
Recombinations as $\Delta\to\Delta_A$:
\vspace{-.5\baselineskip}
\begin{align*}
  L[0]_\Delta & \to A_2[0]_{\Delta_A}\oplus L[0]_{\Delta_A+1}, \\
  L[j\geq 1]_\Delta & \to A_1[j]_{\Delta_A}\oplus L[j-1]_{\Delta_A+1/2}.
\end{align*}
\vspace{-1.5\baselineskip}

Absolutely protected: $B_1[0]_0$ unit operator.


\paragraph{3d \(\Nsusy=2\) algebra \(\lie{osp}(2|4)\).}
Supercharges $Q\in[1]^{(-1)}_{1/2}$ and $\Qbar\in[1]^{(1)}_{1/2}$.
Let $\Delta_A=\frac{1}{2}j-r+1$ and $\Delta_B=-r$ and $\Delta_{\Abar}=\frac{1}{2}j+r+1$ and $\Delta_{\Bbar}=r$.
Multiplets $L\Lbar[j]^{(r)}_{\Delta>j/2+\abs{r}+1}$, one-sided short $L\Abar[j]^{(r>0)}_{\Delta_{\Abar}}$, $L\Bbar[0]^{(r>1/2)}_r$, $A\Lbar[j]^{(r<0)}_{\Delta_A}$, $B\Lbar[0]^{(r<-1/2)}_{-r}$, and two-sided $A\Abar[j]^{(0)}_{j/2+1}$, $A\Bbar[0]^{(1/2)}_{1/2}$, $B\Abar[0]^{(-1/2)}_{1/2}$, $B\Bbar[0]^{(0)}_0$.
Here $(A,B,\Abar,\Bbar)$ mean $(A_\ell,B_1,\Abar_\ell,\Bbar_1)$ with $\ell=1+\delta_{j0}$.
Recombinations as $\Delta\to j/2+\abs{r}+1$:
\vspace{-.8\baselineskip}
\begin{align*}
  L\Lbar[j]^{(r<0)}_\Delta
  & \to A\Lbar[j]^{(r)}_{\Delta_A} \oplus
    \left\{ \begin{array}{@{}ll@{}}
              B\Lbar[0]^{(r-2)}_{\Delta_A+1} & \text{if $j=0$} \\
              A\Lbar[j-1]^{(r-1)}_{\Delta_A+1/2} & \text{if $j\geq 1$}
            \end{array} \right\} ,
  \\
  L\Lbar[j]^{(r>0)}_\Delta
  & \to L\Abar[j]^{(r)}_{\Delta_{\Abar}} \oplus \left\{ \begin{array}{@{}ll@{}}
    L\Bbar[0]^{(r+2)}_{\Delta_{\Abar}+1} & \text{if $j=0$} \\
    L\Abar[j-1]^{(r+1)}_{\Delta_{\Abar}+1/2} & \text{if $j\geq 1$} \\
  \end{array} \right\} , \\
  L\Lbar[j]^{(0)}_\Delta
  & \to A\Abar[j]^{(0)} \oplus
    \left\{\begin{array}{@{}l@{}}
             B\Lbar[0]^{(-2)} \oplus L\Bbar[0]^{(2)} \ \text{if $j=0$}\\
             A\Lbar[j-1]^{(-1)} \oplus L\Abar[j-1]^{(1)}
           \end{array}\right\} .
\end{align*}
\vspace{-\baselineskip}

Absolutely protected: $B\Bbar$ (unit operator).
Absolutely protected if R-charges are quantized: free chiral $A\Bbar$, chirals $L\Bbar_1[0]^{(r)}_r$ with $\frac{1}{2}<r<2$, and their conjugates.

\paragraph{3d \texorpdfstring{\(\Nsusy\geq 3\)}{N>2} algebra \(\lie{osp}(\Nsusy|4)\).}
Supercharges $Q\in[1]^{(R_Q)}_{1/2}$ with $R_Q=(1,0,\dots)$ for $\Nsusy\geq 5$, $R_Q=(1,1)$ for $\Nsusy=4$, $R_Q=(2)$ for $\Nsusy=3$.
Let $\Delta_A=\frac{1}{2}j+h_1+1$ and $\Delta_B=h_1$ with
$h_1=R_1+\dots+R_{k-2}+\frac{1}{2}R_{k-1}+\frac{1}{2}R_k$ for $\Nsusy=2k$
and same with $R_k\to 0$ for $\Nsusy=2k-1$.
Multiplets: $L[j]^{(R)}_{\Delta>\Delta_A}$, $A_{1+\delta_{j0}}[j]^{(R)}_{\Delta_A}$, $B_1[0]^{(R)}_{\Delta_B}$.
Recombinations as $\Delta\to\Delta_A$:
\vspace{-.5\baselineskip}
\begin{align*}
  L[0]^{(R)}_\Delta & \to A_2[0]^{(R)}_{\Delta_A}\oplus B_1[0]^{(R+2R_Q)}_{\Delta_A+1}, \\
  L[j\geq 1]^{(R)}_\Delta & \to A_1[j]^{(R)}_{\Delta_A}\oplus A_{1+\delta_{j1}}[j-1]^{(R+R_Q)}_{\Delta_A+1/2},
\end{align*}
\vspace{-\baselineskip}

Absolutely protected: $B_1[0]^{(R)}$ when $R-2R_Q$ has some negative components (is not a dominant weight).
...
\paragraph{3d \(\Nsusy=2\)} \arxiv{hep-th/9703110,1305.1633}
Data: gauge group~$G$, Chern--Simons levels, chiral multiplets $\phi$ in a complex representation~$V$ of~$G$,
holomorphic superpotential $W(\phi)$.  Gauge couplings run: $g\sim M^{1/2}$.

In SCFT, $\dim(X)\geq R_{\text{IR}}(X)$, equality for $X$ (anti)chiral.
The R-symmetry can mix with (possibly emergent) Abelian flavour symmetry.
...
\paragraph{Exotic branes.}
In type II, denote $b_n^{(\dots,d,c)}$ a brane whose mass scales like $l_s^{-1} g_s^{-n} \prod_{i=1}^{9} (R_i/l_s)^{p_i}$ in terms of the radii~$R_i$,
where $b=\#\{p_i=1\}$, $c=\#\{p_i=2\}$, $d=\#\{p_i=3\}$, \dots{}
E.g.\@, $\text{F1}\equiv 1_0$, $\text{D3}\equiv 3_1$, $\text{NS5}\equiv 5_2$, $\text{KK}\equiv 5_2^1$.
Many exotic branes are codimension $\leq 2$, non-geometric, defined using duality transformations, typically have $n\geq 3$ except the $7$-brane $5_2^2$.
Extensive taxonomy in \arxiv{1806.00430} (their Figure 1 lists S/T-duality orbit of F1).
...
\subsection{5d dualities}

\arxiv{1909.05250}

\subsection{4d \(\Nsusy=2,4\) S-duality}

S-duality of 4d $\Nsusy=4$ SYM (exact).

S-duality of 4d $\Nsusy=2$ gauge theories (exact), see \arxiv{0904.2715}.

\subsection{4d \(\Nsusy=1\) conformal dualities}

\subsection{4d \(\Nsusy=1\) IR dualities}
...
Gauge-invariant chirals $\Operator$ of trial R-charge $\leq 2/3$ are free and of true IR R-charge $2/3$ thanks to mixing with emergent $\lie{u}(1)$ acting on~$\Operator$.
...
\subsection{3d \(\Nsusy=4\) dualities}

3d $\Nsusy=4$ mirror symmetry exchanging Coulomb and Higgs branches.
Self-mirror $T[SU(N)]$.

\subsection{3d \(\Nsusy=2\) IR dualities}

3d $\Nsusy=2$ Aharony \hepth{9703215}, Giveon--Kutasov, Aharony--Fleischer dualities (IR).

3d $\Nsusy=2$ mirror symmetry exchanging Coulomb and Higgs branches.

$U(N)$ with $N_f(\square\oplus\overline{\square})$ and $W=M_++M_-$ (monopoles)
dual to same with $N'=N_f-N-2$ and dual mesons \arxiv{1703.08460}

\subsection{3d non-supersymmetric IR dualities}
...
$T_{\dots[\mu_1\dots\mu_m]\dots}^{\dots}=\sum_{\sigma\in S_m}T_{\dots\mu_{\sigma(1)}\dots\mu_{\sigma(m)}\dots}^{\dots}/m!$ symmetrization;
antisymmetrization $T_{\dots[\mu_1\dots\mu_m]\dots}^{\dots}$ has extra $\epsilon(\sigma)=\pm 1$ (signature of the permutation).
Derivatives: $\partial_\mu=\partial/\partial x^\mu$ and $T^{\nu_1\dots\nu_n}_{\rho_1\dots\rho_r\,,\mu_1\dots\mu_m}=\partial_{\mu_1}\cdots\partial_{\mu_m} T^{\nu_1\dots\nu_n}_{\rho_1\dots\rho_r}$
while ``;'' stands for $\nabla$ as $T^{\nu_1\dots\nu_n}_{\rho_1\dots\rho_r\,;\mu_1\dots\mu_m}=\nabla_{\mu_1}\cdots\nabla_{\mu_m} T^{\nu_1\dots\nu_n}_{\rho_1\dots\rho_r}$.

\paragraph{Connection\texorpdfstring{ $\nabla$}{}}${}=\partial+\Gamma$ in terms of Christoffel symbols~$\Gamma$.
...
Differences $\Gamma-\Gamma'$ of connections are tensors.

\paragraph{Torsion of a connection:} $T^\lambda{}_{\mu\nu}=2\Gamma^\lambda{}_{[\mu\nu]}$

\paragraph{Riemann tensor} $R(u,v)w=\nabla_u\nabla_vw-\nabla_v\nabla_uw-\nabla_{[u,v]}w$.
For a torsion-free connection,
$R^\lambda{}_{\mu\nu\rho}=2\del_{[\nu}\Gamma^\lambda{}_{\rho]\mu}+2\Gamma^\lambda{}_{\tau[\nu}\Gamma^\tau{}_{\rho]\mu}$
obeys
$R^\lambda{}_{\mu(\nu\rho)}=0$, $R^\lambda{}_{[\mu\nu\rho]}=0$ and $R^\lambda{}_{\mu[\nu\rho;\tau]}=0$ (``Bianchi'').

\paragraph{Ricci tensor} $R_{\mu\nu}=R^\lambda{}_{\mu\lambda\nu}$

\paragraph{Metric \texorpdfstring{$g$}{g}}${}\in S^2(T^*M)$,
raise/lower indices as $v_\mu=g_{\mu\nu}v^\nu$ and $v^\nu=g^{\nu\rho}v_\rho$ using $g_{\mu\nu}$ and inverse metric $g^{\mu\nu}$.
Notation $\abs{g}=\abs{\det g}$

\paragraph{Levi-Civita connection of \texorpdfstring{$g$}{g}:}
unique torsion-free connection with $\nabla g=0$.
Given by $\Gamma^\lambda{}_{\nu\rho}=\tfrac{1}{2}g^{\lambda\mu}(\partial_\rho g_{\mu\nu}+\partial_\nu g_{\mu\rho}-\partial_\mu g_{\nu\rho})$.
One can show $\sqrt{\abs{g}}\,\nabla_\nu T^{[\nu\mu_2\dots]} =\partial_\nu(T^{[\nu\mu_2\dots]}\sqrt{\abs{g}})$ is a total derivative (using $\Gamma^\lambda{}_{\lambda\rho}=\partial_\rho\log\sqrt{\abs{g}}$).

\paragraph{Killing vector} $k_\mu$ such that $\nabla_{(\mu} k_{\nu)}=0$.
For a symmetric conserved stress-tensor~$T$ we have $\nabla_\mu(k_\nu T^{\mu\nu})=0$, giving conserved quantities.

\paragraph{Riemann and Ricci tensors} of the Levi-Civita connection
obey $R_{\kappa\mu\nu\rho}=-R_{\mu\kappa\nu\rho}=R_{\nu\rho\kappa\mu}$ and $R_{\mu\nu}=R_{\nu\mu}$.
Ricci scalar $R=R_{\mu\nu}g^{\mu\nu}$.
Einstein tensor $G_{\mu\nu}=R_{\mu\nu}-Rg_{\mu\nu}/2$ obeys $\nabla_\mu G^{\mu\nu}=0$ (``contracted Bianchi identity'').
Amusing formula $R_{\lambda\mu\nu\rho}=2\del_{[\nu|}\Gamma_{\lambda|\rho]\mu}-2\Gamma_{\tau\lambda[\nu}\Gamma^\tau{}_{\rho]\mu}$
(used for Gauss' equation).

\paragraph{Einstein equations} $G_{\mu\nu}+\Lambda g_{\mu\nu}=(8\pi G/c^4)T_{\mu\nu}$
with cosmological constant~$\Lambda$.

\paragraph{Wave gauge} sets $g^{\mu\nu}\nabla_\mu\nabla_\nu x^\lambda=\Gamma^\lambda{}_{\mu\nu}g^{\mu\nu}$ to $0$.
Called harmonic gauge or harmonic coordinates in the Riemannian case.

\paragraph{Submanifold \texorpdfstring{$\Sigma\subset M$}{Sigma in M}:}
first fundamental form${}={}$induced metric $g_\Sigma(u,v)=g(u,v)$ for vectors $u,v\in T\Sigma\subset TM$;
second fundamental form (read ``two'') $\mathrm{II}(u,v)=(\nabla_uv)^{\perp}=(\nabla_vu)^{\perp}$ is a quadratic form $T\Sigma\times T\Sigma\to N\Sigma$ valued in normal bundle.
Components are $\Gamma_{nij}$ with $n\perp\Sigma$, $i,j\parallel\Sigma$.

\paragraph{Submanifold given by \texorpdfstring{$f\colon\Sigma\to M$}{f:Sigma->M}} has $g^\Sigma_{ij}=\del_if^\mu\del_jf^\nu g_{\mu\nu}$
and second fundamental form $\mathrm{II}_{\mu ij}=\Gamma_{\mu\nu\rho}\del_if^\nu\del_jf^\rho$.

\paragraph{Gauss equation}
$R_{ijkl}=R^\Sigma_{ijkl}+2\mathrm{II}_\mu{}_{i[k}\mathrm{II}^\mu{}_{l]j}$ (sign?).

\paragraph{Hypersurfaces} also have a scalar-valued second fundamental form $K_{ij}=n^\mu\mathrm{II}_{\mu ij}$ with $n$ a unit normal (sign ambiguous).

\paragraph{Codazzi--Mainardi equation} (for $\Sigma\subset M$ a hypersurface)
$n^\mu R^\Sigma_{\mu ijk}=2K_{i[j;k]}$ (sign?).

\paragraph{Einstein constraints}
$R-K_{ij}K^{ij}+(K_i^i)^2=2\rho+2\Lambda$ and
$\nabla^j K_{ij}-\del_i(K_j^j)=J_i$
...
\paragraph{Plethystic exponential.}  Let $\mathbf{m}\subset R[[x_1,\ldots,x_n]]$ be series with no constant term over a ring~$R$.  Then $\plexp\colon\mathbf{m}\to 1+\mathbf{m}$ obeys $\plexp[x_i^p]=1/(1-x_i^p)$, $\plexp[f+g]=\plexp[f]\plexp[g]$ and $\plexp[\lambda f]=\plexp[f]^\lambda$ for $\lambda\in R$.  It maps an index of single-particle states $f(x)$ to that of multiparticle states $\plexp[f]=\exp\sum_{k\geq 1}\frac{1}{k}f(x_1^k,\ldots,x_n^k)$.

\paragraph{Plethystic logarithm} $\pllog$ defined by $\plexp\circ\pllog=\id$ and $\pllog\circ\plexp=\id$ hence
$\pllog[1-x_i^p]=-x_i^p$ and $\pllog[fg]=\pllog[f]+\pllog[g]$ and $\pllog[f^\lambda]=\lambda\pllog[f]$ for $\lambda\in R$.
More generally, $\pllog[f]=\sum_{k\geq 1}\frac{1}{k}\mu(k)\log f(x_1^k,\ldots,x_n^k)$ where
$\mu$~is the M\"obius function: $\mu(a^2b)=0$ for integers $a>1$ and~$b$, and $\mu(p_1\dots p_m)=(-1)^m$ for distinct primes~$p_i$.
