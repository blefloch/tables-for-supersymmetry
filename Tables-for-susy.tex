...
\cs_set:Npn \@sect #1 #2 #3 #4 #5 #6 [#7] #8
  {
    ...
    \tl_set:Nx \l__my_label_tl
      {
        \exp_last_unbraced:Nf \__my_alphanumeric:N
...
$\zeta(s)=\Li_s(1)$ where
$\Li_s(z)=\sum_{k\geq 1}z^k/k^s=\bigl(1/\Gamma(s)\bigr)\int_0^\infty t^{s-1}\mathrm{d}t/(e^t/z-1)$ obeys
...
\paragraph{Gravitational anomaly} in $4k+2$ dimensions for Weyl fermions (spin $\tfrac{1}{2}$ or $\tfrac{3}{2}$) or self-dual $2k+1$ form.
...
$\RR^{4|4}\ni(x^m|\theta^\alpha,\overline{\theta}^{\dot{\alpha}})$ in $(2,2)\oplus(2,1)\oplus(1,2)$ of $\lie{so}(1,3)$ or $\lie{so}(4)=\lie{su}(2)^2$.
Supercharges
$Q_\alpha=\del_{\theta^\alpha}-i\sigma^m_{\alpha\dot{\alpha}}\overline{\theta}^{\dot{\alpha}}\del_{x^m}$ and
$\overline{Q}_{\dot{\alpha}}=-\del_{\overline{\theta}^{\dot{\alpha}}}+i\sigma^m_{\alpha\dot{\alpha}}\theta^\alpha\del_{x^m}$
...
Note $\overline{Q}_{\dot{\alpha}}=e^{-A}\overline{D}_{\dot{\alpha}}e^{A}$ for $A=2i\theta\sigma^m\overline{\theta}\del_{x^m}$;
...
\tabfoot{1} 10d $(1,0)$ supergravity (``type I'') has a \href{http://inspirehep.net/record/192309}{gravitational anomaly}.
...
\paragraph{\texorpdfstring{$\Lie{SU}(K)$ with $N\square+N\overline{\square}$}{SU(K) with N flavours}} chiral multiplets (``electric theory'')
is dual (Seiberg duality) to $\Lie{SU}(N-K)$ with $N\square+N\overline{\square}$ and $N^2$ gauge-invariant chiral multiplets $Q_\alpha$, $\widetilde{Q}_\beta$, $M^{\alpha\beta}$ with superpotential $W=\Tr\widetilde{Q}QM$ (``magnetic theory'').  RG flow:
\begin{itemize}\setlength{\itemsep}{0pt}
\item for $2\leq K\leq N/3$ the original theory is IR-free;
\item for $N/3<K<2N/3$ flow to SCFT;
\item for $2N/3\leq K\leq N-2$ the dual theory is IR-free.
\end{itemize}

% \paragraph{\texorpdfstring{$\Lie{SO}(K)$ with $N\square+N\overline{\square}$}{SO(K) with N flavours}} is dual to $\Lie{SO}(N-K+4)$ with $N\square+N\overline{\square}$ and extra neutral coupled by cubic~$W$
% Check if both fundamental and antifundamental!

% \paragraph{\texorpdfstring{$\Lie{USp}(2K)$ with $2N\square+2N\overline{\square}$}{USp(2K) with 2N flavours}} is dual to $\Lie{USp}(2N-2K-4)$ with $2N\square+2N\overline{\square}$ and extra neutral coupled by cubic~$W$
% Check if both fundamental and antifundamental!
