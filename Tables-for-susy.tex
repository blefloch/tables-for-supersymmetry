...
\noindent\textbf{\Large Tables for supersymmetry.} Based on [1-3].
...
\midrule
\multirow{3}{*}{\rotatebox{90}{$\lie{sp}(2n,\ring{R})\!\!$}}
& $\lie{usp}(2n)$ & compact & \\
& $\lie{sp}(2n,\ring{R})$  & $\lie{u}(n)$ & \\
...
\lie{so}(2,1) &=\lie{su}(1,1)=\lie{sl}(2)=\lie{sp}(2,\mathrlap{\ring{R})}\\
...
$\lie{osp}(m|2n)$ & $\lie{so}(m,\ring{C}) \oplus \lie{sp}(2n,\ring{R})$ & $(m,2n)$ \\
...
\midrule
\lie{osp}(m-p,p|2n) & \lie{so}(m-p,p)\oplus \lie{sp}(2n,\ring{R}) \\
...
\bullet \text{\quad is} & \ring{R}& \ring{R}\oplus\ring{R} & \ring{R} & \ring{C} & \ring{H} & \ring{H}\oplus\ring{H} & \ring{H} &\ring{C}\\
\end{tab}
...
$n=\dim_{\ring{R}}X$) is a $G$-subbundle of the
$\Lie{GL}(n,\ring{R})$-principal bundle $\Lie{GL}(TX)$ of tangent
frames, namely a global section of $\Lie{GL}(TX)/G$.

A manifold is oriented if it has a $\Lie{GL}^+(n,\ring{R})=\{\det>0\}$
...
\begin{tab}{@{ }l@{ \ }l@{ \ }l@{ }}
  ...
  Almost symplectic & $\Lie{Sp}(2n/2,\ring{R})$ & Non-degenerate $\omega\in\Omega^2X$\\
  Almost complex & $\Lie{GL}(n/2,\ring{C})$ & $J:TX\to TX$, $J^2=-1$ \\
...
Since $\Lie{GL}(n,\ring{R})/O(n)$ is contractible, any manifold admits
...
$\Lie{GL}(n/2,\ring{C})/U(n/2)$ is contractible so almost complex
manifolds admit almost Hermitian structures.

\paragraph{A $G$-structure is $k$-integrable} (resp.\@ integrable) near
$x\in X$ if it can be trivialized to order~$k$ (resp.\@ all orders) in a
neighborhood of~$x$.  We automatically have $0$-integrability.
...
An almost complex structure is complex if (equivalently) it is integrable; it is $1$-integrable; it has a vanishing Nijenhuis tensor $N_J:\Lambda^2 X\to TX$ defined on vector fields $u$, $v$
by the Lie brackets $N_J(u,v)=-J^2[u,v]+J[Ju,v]+J[u,Jv]-[Ju,Jv]$;
the Lie bracket of $(1,0)$ vector fields is a $(1,0)$ vector field;
$\dd{}=\partial+\bar{\partial}:\Omega^{(p,q)}X\to\Omega^{(p+1,q)}X\oplus\Omega^{(p,q+1)}X$ has no $(p+2,q-1)$ or $(p-1,q+2)$ components;
or $\bar{\partial}^2=0$.
...
\begin{tikzpicture}
  ...
  \path[-stealth]
    (m-1-1) edge node [above] {$\omega$} (m-1-3)
    (m-1-1) edge node [left] {$\exists g$} (m-2-2)
    (m-1-1) edge node [left] {$J$} (m-3-1)
    (m-1-3) edge node [above] {$\dd{\omega}=0$} (m-1-5)
    (m-1-3) edge node [right] {$J$/$g$} (m-3-3)
    (m-1-5) edge node [right] {$J$} (m-3-5)
    (m-2-2) edge node [left] {$J$/$\omega$} (m-3-3)
    (m-3-1) edge node [below] {$\omega$/$g$} (m-3-3)
            edge node [right] {$N_J=0$} (m-5-1)
    (m-3-3) edge node [above] {$\dd{\omega}=0$} (m-3-5)
            edge node [right] {$N_J=0$} (m-5-3)
    (m-3-5) edge node [right] {$N_J=0$} (m-5-5)
    (m-5-1) edge node [above] {$\omega$} (m-5-3)
    (m-5-3) edge node [above] {$\dd{\omega}=0$} (m-5-5)
    ;
  \pgfresetboundingbox
  \path [use as bounding box] (m-3-1.west |- m-1-3.north) rectangle (m-1-5.east |- m-5-3.south);
\end{tikzpicture}
\end{tab}
...
% $\Lie{G}_2\subset O(7)$.  Equivalently it is a Riemannian $7$-manifold
% with special holonomy $\subseteq\Lie{G}_2$.
...
space, \href{https://ncatlab.org/nlab/show/Berger\%27s+theorem}{Berger's
  theorem} allows
\begin{tab}{lll}
  Holonomy & Manifold type & dim\\
  \midrule
  $\Lie{SO}(n)$ & orientable & $n$ \\
  $\Lie{U}(m)$ & K\"ahler & $2m$ \\
  $\Lie{SU}(m)$ & Calabi--Yau & $2m$ \\
  $\bigl(\Lie{Sp}(k)\times\Lie{Sp}(1)\bigr)/\ring{Z}_2$ & quaternionic K\"ahler & $4k$ \\
  $\Lie{Sp}(k)$ & hyper-K\"ahler & $4k$ \\
  $\Lie{Spin}(7)$ & $\Lie{Spin}(7)$ manifold & $8$ \\
  $\Lie{G}_2$ & $\Lie{G}_2$ manifold & $7$ \\
\end{tab}
% todo: what Sp?
% more at https://en.wikipedia.org/wiki/Holonomy
% perhaps http://projecteuclid.org/euclid.jdg/1090351387 interesting?

% more on http://mathworld.wolfram.com/Hyper-KaehlerManifold.html
