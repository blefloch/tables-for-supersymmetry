...
\paragraph[Real simple Lie algebras]{A real simple Lie algebra} can be the complex algebras above or their real forms.
Let
$\lie{su}^*(2n)=\lie{gl}(n,\HH)/\RR$,
...
\begin{tab}{*{3}{>{$}l<{$}}}\label{page:ADE-polynomial}
  \lie{G} & (p,q,r) & \multicolumn{1}{p{3cm}}{Kleinian polynomial} \\
...
Every compact connected Lie group~$K$ is a quotient of $\Lie{U}(1)^n\times\prod_{i=1}^m K_i$ by a finite subgroup of its center, where~$K_i$ are simple, compact, simply-connected, connected Lie groups.
...
Similarly, for $j<4n+2$, $\pi_j(\Lie{USp}(2n))$ is $\ZZ$~for $j\equiv 3,7\bmod{8}$, $\ZZ_2$~for $j\equiv 4,5\bmod{8}$, and $0$~otherwise.
...
% todo: say somewhere $SU(2)$ is $S^3$.

% todo: fundamental groups.  Include $\pi_1(\Lie{U}(n))=\ZZ$.
