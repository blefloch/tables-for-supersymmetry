...
and the center of~$K$ is $Z(K)=\bigl(U(1)^n\times\prod_{i=1}^m Z(K_i)\bigr)/\Gamma$.
...
For each real simple Lie algebra~$\lie{g}$, we write: $G_{\text{cf}}$~as a quotient of its algebraic universal cover~$\widetilde{G}_{\text{alg}}$ (largest embeddable in $\Lie{GL}(N<\infty)$) by the algebraic~$\pi_1$; the (topological)~$\pi_1$; the real rank~$r_{\Re}$; and~$K$.
...
$\pi_0(\Lie{O}(p,q))=\pi_0(\Lie{O}(p))\times\pi_0(\Lie{O}(q))$ is $\ZZ_2^2$ for $p,q\geq 1$;
...
% todo: say somewhere $SU(2)$ is homeomorphic to $S^3$.
...
\paragraph{For \(8\) supercharges}, vector and hyper; in 2d $\Nsusy=(4,4)$ also twisted hyper.

\paragraph{For \(4\) supercharges}, vector, chiral, linear multiplets; in 2d $\Nsusy=(2,2)$ also twisted chirals, semichirals, \ldots{}
...
A gauge group is a compact reductive Lie group~$G$ such as $(SU(3)\times SU(2)\times U(1))/\ZZ_6$.
Gauge couplings are one real parameter per simple factor in~$\lie{g}$.
...
\subsection{5d \(\Nsusy=1\) SCFTs} built from 5-brane diagrams or UV fixed point of gauge theory.

$\Lie{SU}(2)$ SYM with $\Nf\leq 7$ fundamental hypermultiplets has $\Lie{SO}(2\Nf)\times\Lie{U}(1)_T\subset\Lie{E}_{\Nf+1}$ flavor symmetry enhancement.
For $\Nf=0$, non-trivial ``$\theta$'' in $\pi_4(\Lie{SU}(2))=\ZZ_2$ gives the $\widetilde{E}_1$~theory with $\Lie{U}(1)_T$~symmetry only.

\subsection{4d \(\Nsusy=4\)}

Data: gauge group, and for each simple fact a gauge coupling and theta angle: $\tau=\theta/(2\pi)+4\pi i/g^2$.

\subsection{4d \(\Nsusy=2\)}

Data: gauge group, representation for half-hypermultiplets. % todo: symplectic rep?

There can be no continuous flavor symmetry enhancement.
...
\paragraph{\((G,G')\) Argyres--Douglas theories} (with $G$ and~$G'$ among $\Lie{A}_k$, $\Lie{D}_k$, $\Lie{E}_{6,7,8}$) are engineered as IIB strings on three-fold singularity $f_G(x_1,x_2)+f_{G'}(x_3,x_4)=0$ where $f_{A_k}(x,y)=x^2+y^{k+1}$ \etc{} (see \autopageref{page:ADE-polynomial}).

\subsection{4d \(\Nsusy=1\)}
...
\paragraph{Pure supersymmetric Yang--Mills (SYM)} classically has $\Lie{U}(1)_R$ symmetry, broken by instantons to $\ZZ_{2h}$ with $h=C_2(\text{adj})$.  It confines, is mass-gapped, and has $C_2(A)$ vacua associated to breaking $\ZZ_{2h}$ to $\ZZ_2$ by gaugino condensation $\langle\lambda\lambda\rangle$.  Witten index $\Tr(-1)^F=h$.

\subsection{3d \(\Nsusy=4\)}

Gauge group~$G$ and
finite-dimensional symplectic representation~$\repr{M}$ of~$G$.
%% according to Kodera-Nakajima 1608.00875

\subsection{3d \(\Nsusy=2\)}

Data: gauge group~$G$, quantized trace on $\lie{g}$ for the
Chern--Simons term, representation~$V$ of~$G$ for chiral multiplets,
$G$-invariant superpotential (may break R-symmetry).

\subsection{1d \(\Nsusy=4\)}
...
R-symmetry: $SU(2)$, times $U(1)$ if $W$~has charge~$2$.  Mixing with flavour symmetries not fixed by superconformal algebra.


\subsection{1d \(\Nsusy=2\)}
...
flavour symmetry group $G_F\subseteq U(V)\times U(M_0)\times U(M_1)$ commuting with~$G$.
Gauge anomaly cancellation: $M\otimes\det^{1/2}V$ must be a representation of~$G$.

Continuous data: gauge couplings, FI parameters, flavor Wilson line and real mass $v,\sigma\in\lie{g}_F$ that commute, $\lie{g}$-equivariant holomorphic odd map $Q\colon V\to \End M$ with $Q^2=0$ describing how supercharges act on~$M$.
...
R-symmetry: $U(1)$ if $Q\colon V\to \End M$ has charge~$1$.  Mixing with flavour symmetries not fixed by superconformal algebra.
...
4-manifold is hyperK\"ahler if and only if it is a gravitational instanton.
...
$\Gamma\subset SU(2)$.  The metric is fixed (up to isometry) by
