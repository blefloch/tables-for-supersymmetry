...
Variant $\thetafunction_1(z;q)=\thetafunction_1(\tau|u)=\I z^{-1/2}q^{1/12}\eta(\tau)\thetafunction(z;q)=-\I z^{1/2}q^{1/8}(q;q)_\infty(qz;q)_\infty(\frac{1}{z};q)_\infty$
with $z=\rme^{2\pi\I u}$.
...
Field strength $W_\alpha=-\frac{1}{4}\overline{D}\overline{D}\rme^{-V}D_\alpha \rme^V$ and $\overline{W}_\alpha=-\frac{1}{4}DD\rme^{-V}\overline{D}_\alpha \rme^V$.
...
\paragraph{3d \(\Nsusy=2\):} $Z = \int_{\lie{T}} \dd{u} \frac{\prod_{\alpha\,\text{root}} (2\sinh(\alpha u/2))^2}{\prod_{w\in\mathcal{R}}\cosh(wu/2)} \rme^{\I k\Tr u^2/(4\pi)}$.

\section{Dualities}

\subsection{4d $\Nsusy=1$ dualities}

\paragraph{Evidence:} global symmetries, 't~Hooft anomaly matching, moduli space of vacua, chiral rings, behaviour of these under F-term deformations,
superconformal index ($S^3_b\times S^1$ partition function).

\paragraph[Seiberg duality: \(\Lie{SU}(C)\) with \(F\) flavours]{Seiberg duality} \href{http://arXiv.org/abs/hep-th/9411149}{[hep-th/9411149]}
...
and $\lie{u}(1)_R$ charges $\frac{F-C}{F}$, $2\frac{F-C}{F}$, $\frac{C}{F}$.
RG flow:
\vspace{-1ex}
...
\end{itemize}
\undovskip
Gauge-invariants match: mesons $M^j{}_i=\tilde{Q}^j Q_i$ and,
for $\Acal\subset\llbracket 1,F\rrbracket$ with $\cardinal{\Acal}=C$,
baryons $B_\Acal=\Wedge_{i\in\Acal} Q_i=\Lambda^{?}\Wedge_{i\not\in\Acal} \tilde{q}^i$ and
antibaryons $\tilde{B}_\Acal=\Wedge_{j\in\Acal} \tilde{Q}^j=\Lambda^{?}\Wedge_{j\not\in\Acal} q_j$.
...
\paragraph[\(\Lie{SO}(C)\) with \(F\) flavours]{SO:}
\mathtikz[inner sep=1pt]{
  \node (A) at (0,0) [rectangle,draw] {$\Lie{SU}(F)$};
...
Non-anomalous flavour symmetry $(\Lie{SU}(F)\times\ZZ_{2F})/\ZZ_{F,\text{diag}}$;
$Q$, $M$, $q$ are $\overline{\square}$, $\operatorname{sym}^2\overline{\square}$, $\square$ under $\Lie{SU}(F)$
and have $\lie{u}(1)_R$ charges $\frac{F-C+2}{F}$, $2\frac{F-C+2}{F}$, $\frac{C-2}{F}$;
...
% \item for $2F/3\leq C\leq F$ the $\Lie{SU}(C')$ theory is IR-free;
% \item for $0<F<C$
\item $F/3<C-2<2F/3$ flow to SCFT;
\item $2F/3\leq C-2\leq F-2$ the $\Lie{SU}(C')$ theory is IR-free;
...
\paragraph[\(\Lie{USp}(2C)\) with \(F\) flavours]{USp:}
\mathtikz[inner sep=1pt]{
...
\mathtikz[inner sep=1pt]{
  ...
  \node (B) at (1.8,0) [rounded rectangle,draw] {$\Lie{USp}(2C')$};
  \draw[->-=.4] (B) -- (A) node [midway,above] {$\tilde{q}$};
  \draw[->-=.6] (A) to [loop,distance=3ex,in=160,out=-160] node [midway,left] {$M$} (A);
  % \draw[
  % decoration = {markings, mark = at position .6 with {\arrow{>}}, mark = at position .4 with {\arrow{>}}},
  % postaction = {decorate}] (A) to [loop,distance=3ex,in=160,out=-160] node [midway,left] {$M$} (A);
}
...
in the $\Lie{SU}$, $\Lie{SO}$, $\Lie{USp}$ cases for $F=2C,2C-4,2C+2$ respectively,
namely $C(R_{\text{chirals}})=2C(\text{adj})$; adding an adjoint gives $\Nsusy=2$ SCFTs.

\paragraph[\(\Lie{SU}(C)\) with flavours and adjoint]{Kutasov--Schwimmer--Seiberg duality} \href{http://arXiv.org/abs/hep-th/9510222}{[hep-th/9510222]}
For $W=\Tr P(X)=\sum_{j=1}^{k+1}s_{k+1-j}\Tr X^j/j$ (with $s_1=0$)
\unskip\vspace{-1ex}
\mathtikz[inner sep=1pt]{
  \node (A) at (0,0) [rectangle,draw] {$\Lie{SU}(F)$};
  \node (B) at (1.5,0) [rounded rectangle,draw] {$\Lie{SU}(C)$};
...
with $W_{\text{mag}}=\alpha(s)-\Tr P(x)+\frac{1}{\mu^2}\sum_{1\leq i\leq j\leq k} s_{k-j}M_i\tilde{q}x^{j-i}q$
for some function~$\alpha$ and mass~$\mu$, and $C'=kF-C\geq 0$.
Gauge-invariants: mesons $M_j=\tilde{Q}X^{j-1}Q$;
for $\cardinal{\Acal}=C$ in $\llbracket 1,F\rrbracket\times\llbracket 1,k\rrbracket$,
...
and $\Tr X^j/j=\partial W/\partial s_{k+1-j}=\partial W_{\text{mag}}/\partial s_{k+1-j}$ expressed in terms of $\Tr x^i$ (here $\alpha(s)$ matters).

Global symmetries $\lie{su}(F)^2\times\lie{u}(1)_B\times\lie{u}(1)_R$
...
Kutasov--Schwimmer--Seiberg, Brodie, Intriligator--Pouliot, Argyres--Intriligator--Leigh--Strassler, Klebanov cascade, Intriligator--Leigh--Strassler, Kutasov--Lin.
...
The $k$-instanton moduli space for $G=\Lie{SU}(N)$ is a $4Nk$-dimensional \href{http://inspirehep.net/record/128223}{[inspi:128223]} hyperK\"ahler manifold,
in bijection with rank~$N$ framed locally free sheaves on~$\CC\PP^2$ \href{http://inspirehep.net/record/125044}{[inspi:125044]}.
