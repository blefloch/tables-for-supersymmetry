...
\subsection{Simple Lie (super)groups}
...
Every (connected) Lie group deformation-retracts onto its maximal compact subgroup.

For any connected Lie group~$G$, $\pi_1(G)$ is finitely-generated abelian, $\pi_2(G)=0$, $\pi_3(G)\sim\ZZ^{\text{finite}}$ (Iwasawa).

Simple Lie groups~$G$ have $\pi_3(G)=\ZZ$.

Any connected compact Lie group~$G$ is a quotient of $\RR^q\times\prod_i G_i$ by a finite central subgroup, where the compact simple Lie groups~$G_i$ can be taken to be simply-connected (and connected).  Then $\pi_n(G)=\prod_i \pi_n(G_i)$ for $n\geq 2$.

For a rank~$r$ Lie group~$G$, there exists $\prod_{i=1}^r S^{2k_i+1}\to G$ which induces an isomorphism of rational homotopy (or of cohomology) groups:
\begin{gather*}
...
\end{gather*}
% todo: understand rational homotopy groups, can I really factor what's given in http://www.math.binghamton.edu/somnath/Notes/GSS5.pdf ?

However, this is not true of integer homotopy groups.  For instance, $\pi_4(SU(2))=\mathbb{Z}_2$.
...
\paragraph{Long exact sequence for a fiber bundle} $F\hookrightarrow E\twoheadrightarrow B$: $\cdots\to\pi_i(F)\to\pi_i(E)\to\pi_i(B)\to\pi_{i-1}(F)\to\cdots$
